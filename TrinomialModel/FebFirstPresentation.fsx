...
let tableProfiles  , _     =  table9FileName
                              |> getDataContent
...
let savingFolder = @"C:\Users\glddm\Desktop\tablesData\"
...
let profileToString (profile:seq<DepthTime>) =  profile 
                                              |> Seq.map (fun x -> x.ToString())
                                              |> Seq.toArray
                                              |> String.concat("\n")

let getProfilePath idx =
    savingFolder + "profile" + string(idx) + ".csv"

let profileToCsv  idx aProfile = 
    
    let profileName = getProfilePath idx
    let wr = new StreamWriter(profileName)
    aProfile
    |> profileToString
    |> wr.Write
    wr.Close()

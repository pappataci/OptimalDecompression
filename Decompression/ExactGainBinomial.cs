...
{
    /// <summary>
    /// Static class to calculate the exact gain for binary problems
    /// </summary>
    public static class ExactGainBinomial
    {

        public static OPTIMIZATIONTYPE OptimizationType = OPTIMIZATIONTYPE.TIMEOFONSETNOMARGINAL;
        public static bool [ ] RiskTissue;
        ...
        private static Vector Equation89LHS ( )
        {
            ...
            // Set the dummy equation for the non risk compartment.
            for ( int c = 0; c < NodeTissue.NumberOfTissues; c++ )
                if ( !RiskTissue [ c ] )
                    dvSumRcs [ c ] = m_dvGain [ c ];
        ...
        private static Vector Equation90 ( )
        {
            ...
            foreach ( int i in m_Data.FullDCSProfileIndicies )
            {
                ...
                for ( int c = 0; c < NodeTissue.NumberOfTissues; c++ )
                    if ( RiskTissue [ c ] )
                        dvSum [ c ] -= p.Divers * dXi_s *
                                       Math.Min ( 1.0 / DCSUtilities.Constants.Epsilon,
                                           Math.Pow ( dvR [ c ] / ( dXi_s - 1.0 ), 2.0 ) );
            }

            // Set the dummy slope for the non risk compartment.
            for ( int c = 0; c < NodeTissue.NumberOfTissues; c++ )
                if ( !RiskTissue [ c ] )
                    dvSum [ c ] = 1.0;
        ...
        private static double [ , ] Equation91 ( )
        {
            ...
            // Set the dummy slope for use with non risk compartments.
            for (int c1 = 0; c1<NodeTissue.NumberOfTissues; c1++)
                for (int c2 = 0; c2<NodeTissue.NumberOfTissues; c2++)
                    if ( c1 == c2 && !RiskTissue [ c1 ] )
                        dmSum [ c1, c2 ] = 1.0;
        ...
        public static double [ ] Optimize ( )
        {
            ...
            switch ( OptimizationType )
            {
                ...
                default:
                    throw new DCSException ( "Bad OPTIMIZATIONTYPE enumeration value in Decompression.ExactGainBinomial.OptimizeGain" );
            }
            ...
            Vector initialGuess                      = Vector.Create ( m_dvGain );
            ...
            do
            {
                if ( n++ > m_iNumberOfRestarts )
                    throw new DCSException ( "Too many restarts required in Decompression.ExactGainBinomial.OptimizeGain" );

                solver.InitialGuess = initialGuess;
                try
                {
                    Vector solution = solver.Solve ( );
                    m_dvGain = solution.ToArray ( );
                }
                catch ( Exception e )
                {
                    throw new DCSException ( "Solver exception thrown in Decompression.ExactGainBinomial.Optimize" );
                }

                initialGuess = ExactGainBinomial.RandomInitialGainVector ( );
